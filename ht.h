...
public:
    ...
    HASH_INDEX_T next() 
    {
      ...
      HASH_INDEX_T x = (this->start_ + (this->numProbes_ * this->dhstep_))% this->m_;
      this->numProbes_++;
      return x;
    }
};
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    double loadFactor = this->totalsize_ / (double)CAPACITIES[mIndex_];
    if (loadFactor >= this->alpha) {
        resize();
    }

    size_t capacityLimit = (size_t)CAPACITIES[mIndex_];
    if (this->totalsize_ >= capacityLimit) {
        throw std::logic_error("Nowhere to insert");
    }

    HASH_INDEX_T h = hash_(p.first);
    prober_.init(h, CAPACITIES[mIndex_], p.first);
    HASH_INDEX_T result = probe(p.first);
    ...
    if (table_[result] == nullptr) {
        ...
        if (table_[result]->item.first == p.first) {
            if (table_[result]->deleted) {
                this->size_++;
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
    ...
    HASH_INDEX_T h = hash_(key);
    prober_.init(h, CAPACITIES[mIndex_], key);

    for (HASH_INDEX_T result = prober_.next(); result != Prober::npos; result = prober_.next()) {
        if (table_[result] == nullptr) {
            return;
        }
        if (table_[result]->item.first == key && !table_[result]->deleted) {
            table_[result]->deleted = true;
            size_--;
            return; 
        }
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    unsigned int oldCapacity = CAPACITIES[mIndex_];
    for (unsigned int i = 0; i < oldCapacity; i++) {
        ...
        if (!temp->deleted) {
            HASH_INDEX_T newHash = hash_(temp->item.first);
            Prober tempProber = prober_;
            tempProber.init(newHash, CAPACITIES[mIndex_ + 1], temp->item.first);
            for (HASH_INDEX_T result = tempProber.next(); result != Prober::npos; result = tempProber.next()) {
                if (newTable[result] == nullptr) {
                    newTable[result] = temp;
                    break; 
                }
            }
        } else {
